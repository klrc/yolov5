...
Usage:
    ...
    $ python train.py --data coco.c6.yaml --weights 'runs/train/exp5/weights/best.pt' --cfg yolov5n.yaml --img 512 --batch-size 128 --device 0
"""
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    print(f"{colorstr('optimizer:')} {type(optimizer).__name__} with parameter groups " f"{len(g[1])} weight (no decay), {len(g[0])} weight, {len(g[2])} bias")
    del g
