...
from utils.augmentations import Albumentations, augment_hsv, copy_paste, image_osd, letterbox, mixup, random_perspective
from utils.general import DATASETS_DIR, LOGGER, NUM_THREADS, check_dataset, check_requirements, check_yaml, clean_str, cv2, segments2boxes, xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn
...
def create_dataloader(
    path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0, rect=False, rank=-1, workers=8, image_weights=False, quad=False, prefix="", shuffle=False
):
    if rect and shuffle:
...
class LoadImagesAndLabels(Dataset):
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(pool.imap(verify_image_label, zip(self.im_files, self.label_files, repeat(prefix))), desc=desc, total=len(self.im_files), bar_format=BAR_FORMAT)
...
def dataset_stats(path="coco128.yaml", autodownload=False, verbose=False, profile=False, hub=False):
    ...
    for split in "train", "val", "test":
        ...
        stats[split] = {
            ...
            "image_stats": {"total": dataset.n, "unlabelled": int(np.all(x == 0, 1).sum()), "per_class": (x > 0).sum(0).tolist()},
            "labels": [{str(Path(k).name): round_labels(v.tolist())} for k, v in zip(dataset.im_files, dataset.labels)],
        }
